...
def _write_u8(path: Path, arr) -> None:
...
def main() -> int:
    ...
    logger = logging.getLogger("dump_k3_vhsdecode")
    ...
    if getattr(f, "linelocs1", None) is None:
        raise RuntimeError("linelocs1 missing")

    _write_f64(outdir / "python_demod_05.f64", f.data["video"]["demod_05"])
    _write_f64(outdir / "python_linelocs1.f64", f.linelocs1)
    _write_f64(outdir / "python_linelocs2.f64", f.linelocs2 if getattr(f, "linelocs2", None) is not None else f.linelocs1)
    _write_u8(outdir / "python_linebad.u8", f.linebad if getattr(f, "linebad", None) is not None else np.zeros(len(f.linelocs1), dtype=np.uint8))
    ...
    meta = {
        ...
        "linelocs1_count": int(len(f.linelocs1)),
        "linelocs2_count": int(len(f.linelocs2)) if getattr(f, "linelocs2", None) is not None else int(len(f.linelocs1)),
