...
__global__ void k_find_pulses(
    const double* demod_05,      // [num_fields x samples_per_field]
    ...
    int num_fields)
{
    ...
    int field_offset = field * samples_per_field;  // absolute offset in batch
    const double* sig = demod_05 + (size_t)field_offset;
        ...
        if (in_pulse) {
            if (val > threshold) {
                ...
                if (length > 0 && cur_start != 0 && count < max_pulses) {
                    starts[count]  = field_offset + cur_start;  // absolute position
