...
const char* video_profile_name(VideoProfile profile) {
    switch (profile) {
        case VideoProfile::NTSC_525_60_VHS: return "NTSC_525_60_VHS";
        case VideoProfile::PAL_625_50_VHS:  return "PAL_625_50_VHS";
        case VideoProfile::MPAL_525_60_VHS: return "MPAL_525_60_VHS";
    }
    return "unknown";
}
...
VideoSystem video_system_from_profile(VideoProfile profile) {
    switch (profile) {
        case VideoProfile::NTSC_525_60_VHS: return VideoSystem::NTSC;
        case VideoProfile::PAL_625_50_VHS:  return VideoSystem::PAL;
        case VideoProfile::MPAL_525_60_VHS: return VideoSystem::NTSC;
    }
...
const char* tape_speed_name(TapeSpeed speed) {
    switch (speed) {
        case TapeSpeed::SP: return "SP";
        case TapeSpeed::LP: return "LP";
        case TapeSpeed::EP: return "EP";
    }
    return "SP";
}
...
VideoFormat::VideoFormat(VideoProfile prof, double sample_rate_mhz, TapeSpeed speed)
    : profile(prof), system(video_system_from_profile(prof)), tape_speed(speed), sample_rate(sample_rate_mhz * 1e6)
{
    if (profile == VideoProfile::NTSC_525_60_VHS) {
        lines_per_frame  = 525;
    ...
    } else if (profile == VideoProfile::PAL_625_50_VHS) {
        lines_per_frame  = 625;
        ...
        frame_rate       = 29.97;
        fsc              = 3575611.888111;
        chroma_under     = 631337.0;
        luma_carrier     = 3900000.0;
        burst_abs_ref    = 5000.0;
        burst_start_us   = 5.3;
        burst_end_us     = 7.8;
        output_line_len  = 909;
    ...
    if (profile == VideoProfile::NTSC_525_60_VHS || profile == VideoProfile::MPAL_525_60_VHS) {
...
void VideoFormat::print_info() const {
    ...
    if (profile == VideoProfile::PAL_625_50_VHS) sys_name = "PAL";
    if (profile == VideoProfile::MPAL_525_60_VHS) sys_name = "PAL-M";
    fprintf(stderr, "Format: %s (%s, %s) @ %.1f MHz capture rate\n",
            sys_name, video_profile_name(profile), tape_speed_name(tape_speed), sample_rate / 1e6);
